...
def ensure_conda_bag_runtime() -> None:
    """Fail fast with a conda-only install hint instead of assuming a system ROS install."""
    missing = [
        module_name
    ...
    if missing:
        install_cmds = "\n".join(
            PURE_PYTHON_ROSBAG_DEPS[module_name] for module_name in missing
        )
        raise ImportError(
            "Missing pure-Python rosbag dependencies in the active environment.\n"
            "This script does not require ROS1/ROS2 system packages, but it does need these "
            "Python modules installed inside the conda env:\n"
            f"{install_cmds}"
        )


ensure_conda_bag_runtime()
