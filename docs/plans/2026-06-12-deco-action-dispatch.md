...
- [x] 修复 dispatcher 源文件提交遗漏：将根目录开发副本忽略规则从 `DECO/` 锚定为 `/DECO/`，确保 `kuavo_train/.../deco/` 下新增模块可被 Git 跟踪。
- [x] 补齐 `deco.inf_step` 部署字段及 config/model 双字段运行时覆盖，消除 YAML 解析错误。
...
- [x] 干净 checkout 中包含 `action_dispatch.py`，`DECOPolicyWrapper` 不再引用未提交模块。
