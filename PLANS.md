...
> **使用说明**：本计划书记录 Kuavo-DECO 集成方案与任务状态，以适用当前分支的有效章节为准，并遵循用户最新确认的决定。上下文读取、日志与任务状态更新遵循 `AGENTS.md`：按需读取相关有效章节及最近相关日志，不全量加载历史记录；纯文档任务仅检查相关文本，仅在关联计划任务状态变化时更新对应复选框。
