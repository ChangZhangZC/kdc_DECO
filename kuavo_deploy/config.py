...
@dataclass
class ConfigTimingDiagnostics:
    """部署时间诊断开关；记录器只观测，不参与控制决策。"""
...
class ConfigDeco:
    ...
    # None 表示沿用 checkpoint 保存的 Flow Matching 推理步数；正整数仅在部署期覆盖推理循环。
    ...
    timing_diagnostics: ConfigTimingDiagnostics = field(default_factory=ConfigTimingDiagnostics)

    def __post_init__(self) -> None:
        # YAML loader 和 asdict() 会把嵌套 dataclass 转为 dict；在配置边界统一恢复强类型对象。
        if isinstance(self.action_dispatch, dict):
            self.action_dispatch = ConfigActionDispatch(**self.action_dispatch)
        if isinstance(self.timing_diagnostics, dict):
            self.timing_diagnostics = ConfigTimingDiagnostics(**self.timing_diagnostics)

    def validate_action_dispatch_structure(self) -> None:
        # bool 是 int 的子类，必须显式拒绝，避免 YAML true 被误解释为 1 个推理步。
        if self.inf_step is not None and (
            isinstance(self.inf_step, bool) or not isinstance(self.inf_step, int) or self.inf_step <= 0
        ):
        ...
        diagnostics = self.timing_diagnostics
        if not isinstance(diagnostics, ConfigTimingDiagnostics):
            raise ValueError("deco.timing_diagnostics must be a mapping/object.")
        if not isinstance(diagnostics.enabled, bool):
            raise ValueError("deco.timing_diagnostics.enabled must be boolean.")
        if (
            isinstance(diagnostics.flush_every_steps, bool)
            or not isinstance(diagnostics.flush_every_steps, int)
            or diagnostics.flush_every_steps <= 0
        ):
            raise ValueError("deco.timing_diagnostics.flush_every_steps must be a positive integer.")

    def validate_action_dispatch_timing(self, dataset_hz: int, chunk_size: int, env: ConfigEnv) -> None:
        """在 checkpoint 加载后校验数据时间语义、控制频率和 chunk 边界。"""
        ...
        if queue_steps is not None and queue_steps > max_actions:
            raise ValueError(
                "deco.action_dispatch.stride_action.queue_steps cannot exceed the number of strided actions "
                f"({max_actions})."
            )
    ...
    if 'env' in cfg and 'inference' in cfg:
        ...
        for k, v in cfg.items():
            if k in env_fields:
                ...
                deco_cfg[k] = v
            elif k == "n_action_steps":
                # 兼容旧 flat deploy YAML；后续统一迁移到 deco.action_dispatch.receding_horizon。
                deco_cfg[k] = v
                ...
                env_cfg[k] = v
    legacy_n_action_steps = deco_cfg.pop("n_action_steps", None)
    has_explicit_action_dispatch = "action_dispatch" in deco_cfg
    is_deco_deploy = (
        inf_cfg.get("policy_type") == "deco"
        or str(env_cfg.get("state_layout", "")).startswith("deco_")
        or bool(deco_cfg)
    )
    if legacy_n_action_steps is not None and has_explicit_action_dispatch:
        raise ValueError(
            "Legacy deco.n_action_steps cannot be combined with deco.action_dispatch. "
            "请只保留新的 action_dispatch 配置。"
        )
    if not has_explicit_action_dispatch:
        # 旧部署 YAML 不再回退到 Stride Action；迁移后统一使用连续动作 Receding Horizon。
        deco_cfg["action_dispatch"] = {
            "mode": "receding_horizon",
            "receding_horizon": {"n_action_steps": legacy_n_action_steps},
        }
        if is_deco_deploy:
            warnings.warn(
                "DECO deploy config has no deco.action_dispatch; defaulting to receding_horizon. "
                "Legacy Stride Action is no longer enabled implicitly.",
                FutureWarning,
                stacklevel=2,
            )
