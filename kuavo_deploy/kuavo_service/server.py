...
def load_policy_from_config(cfg: KuavoConfig):
    ...
    if policy_type == "diffusion":
        ...
        runtime_info = configure_deco_runtime(policy, cfg.deco, cfg.env)
        print(
            "DECO runtime configured: "
            f"inf_step={runtime_info['inf_step']} "
            f"source={runtime_info['inf_step_source']}; "
            f"action_dispatch={runtime_info['action_dispatch']}"
        )
    else:
        raise ValueError(
