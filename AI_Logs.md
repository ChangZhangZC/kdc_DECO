...
- **任务**: 根据用户要求，在多相机 RGB-D 前端分支上执行 `git merge deco/fix/action-state`，将后端 Receding Horizon / Temporal Ensembling / Stride Action 三模式动作分发逻辑合入腕部相机融合前端。
- **冲突处理**:
  - `AI_Logs.md`：保留 `deco/feature/muti-visual` 的 2026-06-21/2026-06-17 多相机前端日志，同时保留 `deco/fix/action-state` 的 2026-06-12/2026-06-11 动作分发与分支整理日志，并新增本条合并记录。
  - `PLANS.md`：同时保留 v2.1 动作分发计划和 v3.0 多相机 RGB-D 前端计划，避免合并后丢失任一功能分支的完成记录。
  - `configs/policy/deco_config.yaml`：保留 `act_rgbd` 三相机视觉配置、`rgb_keys/depth_keys`、`letterbox_fill_depth=0.5`，同时保留 `dataset_hz/control_hz/action_stride/n_action_steps` 的 checkpoint 兼容字段，并明确正式部署动作分发由 deploy YAML 的 `deco.action_dispatch` 决定。
- **合并后的关键语义**:
  - 推理前端继续按固定顺序读取 head / left wrist / right wrist 三组 RGB-D，并 stack 为 `[B,V,C,H,W]`。
  - 模型入口继续使用 multi-view ACT-style RGB-D encoder，融合后的 camera tokens 进入 DECO 主干。
  - 推理后端由 `kuavo_train/wrapper/policy/deco/action_dispatch.py` 接管动作 chunk 消费方式；默认部署配置为 30Hz Receding Horizon。
- **验证边界**:
  - 本次遵守仓库 No-Runtime 约束，未运行 Python、pytest、训练、转换、仿真、ROS、pip、conda 或任何环境变更命令。
  - 仅执行 Git 合并、冲突标记检索、静态文本检查、`git diff --check` 与人工逻辑审查。
