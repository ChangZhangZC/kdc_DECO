...
### 精简 Agent 协作规则与文档维护流程

- **授权与目的**：用户确认优化方案，减少重复审批、无关上下文读取和过度记录。
- **`AGENTS.md`**：合并重复规则，改为按任务范围授权、按需读取计划和日志、按相关性参考 ACT/DP 与技能；保留执行、删除、补丁隔离及兼容性边界，精简目录地图、中文注释要求与交付标准。
- **`PLANS.md`**：仅调整顶部使用说明，统一上下文读取与状态更新规则；技术方案及任务复选框保持原状。
- **`AI_Logs.md`**：按现有日期倒序增加本条记录，保留历史内容；后续仅记录实际变更及必要检查结论。
- **检查与限制**：核对三个 Markdown 文件的文本差异及 `git diff --check`；本次不涉及代码或运行行为变更，未运行项目代码、测试或安装依赖。
...
### 删除 .gitignore 中过宽的 DECO 目录规则

- **任务**：根据用户明确要求，删除 `.gitignore` 中的 `DECO/` 规则。
- **根因与目的**：
  - Git ignore pattern 默认不区分只匹配仓库根目录还是任意层级同名目录；现有 `DECO/` 规则会误命中 `kuavo_train/wrapper/policy/deco/`。
  - 该误匹配导致新增 DECO wrapper 文件需要使用强制暂存，容易遗漏后续新增模块。
- **修改文件 1**：`.gitignore`
  - 删除唯一一行 `DECO/`，不新增替代规则。
- **修改文件 2**：`PLANS.md`
  - 在当前 3View RGB 分支任务清单中记录该 Git 跟踪规则修复。
- **边界说明**：
  - 未删除任何文件，未修改训练、模型、部署、数据转换或动作后端逻辑。
  - 删除规则后出现的未跟踪文件仅代表 Git 现在能够看到它们，不会在本任务中自动加入版本控制。
- **No-Runtime 边界**：
