...
### 修复部署侧 `inf_step` 配置解析与 `action_dispatch` 模块提交遗漏

- **现场错误**:
  - 部署 YAML 包含 `deco.inf_step`，但部署机器上的旧 `ConfigDeco` 未声明该字段，配置构造阶段抛出 `unexpected keyword argument 'inf_step'`。
  - `DECOPolicyWrapper.py` 已引用 `kuavo_train.wrapper.policy.deco.action_dispatch`，但部署 checkout 中缺少该模块，导入阶段抛出 `ModuleNotFoundError`。
- **根因定位**:
  - `action_dispatch.py` 在本地工作区存在，但没有进入提交 `0da6e15`，Git 历史和远端分支均不存在该文件。
  - `.gitignore` 原规则为 `DECO/`；在 macOS 默认大小写不敏感文件系统上，它会误匹配路径中的小写 `deco/` 目录，使新增 Python 模块被静默忽略，而已经跟踪的旧文件不受影响。
- **修复内容**:
  - 将 ignore 规则锚定为 `/DECO/`，只忽略仓库根目录的原始 DECO 开发副本，不再影响 `kuavo_train/wrapper/policy/deco/`。
  - 将 `kuavo_train/wrapper/policy/deco/action_dispatch.py` 纳入本次变更，确保干净 checkout 包含 Receding Horizon、Temporal Ensembling 和 Stride Action 实现。
  - 在 `ConfigDeco` 中声明并校验 `inf_step`；部署正整数会同步更新 `policy.config.inf_step` 与 `policy.model.inference_step`，`null` 保持 checkpoint 值。
  - 扩展部署配置静态测试，覆盖非法 `inf_step` 类型、config/model 双字段覆盖以及 dispatcher 参数不被改变。
- **验证边界**:
  - 通过 Git 静态检查确认旧提交和远端确实缺少 `action_dispatch.py`，并确认修正后的 ignore 规则不再忽略该文件。
  - 按仓库 No-Runtime 规则未执行 Python、pytest、MuJoCo 或 ROS；部署机器需要在获取包含该新文件和 `ConfigDeco.inf_step` 的完整提交后重新运行。
