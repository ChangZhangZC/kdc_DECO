...
### 2B.1 `inf_step` 部署覆盖优先级

`inf_step` 是 Flow Matching 生成完整 action chunk 时的数值积分/去噪步数，不是训练 epoch、控制频率、chunk 长度或 `n_action_steps`。它保存在 checkpoint policy config 中，但只在推理分支读取，因此部署期调整不需要重新训练。

部署优先级冻结如下：

- `deco.inf_step: null` 或旧 YAML 缺少该字段：同时保留 checkpoint 的 `policy.config.inf_step` 与模型构造时复制得到的 `policy.model.inference_step`。
- `deco.inf_step: <正整数>`：checkpoint 加载完成后，同时覆盖上述两个字段；只修改后续推理循环次数，不修改权重和 action chunk shape。
- 非整数、布尔值、零或负数必须在下发动作前报错。
- 本地仿真、真机和 inference server 必须通过同一个 `configure_deco_runtime()` 入口应用覆盖，并在启动日志中记录最终值及 `checkpoint`/`deploy_override` 来源。
