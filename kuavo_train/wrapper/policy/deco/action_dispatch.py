...
class RecedingHorizonDispatcher(ActionDispatcher):
    ...
    def __init__(self, n_action_steps: int | None) -> None:
        if n_action_steps is not None and n_action_steps <= 0:
            raise ValueError("receding_horizon.n_action_steps must be positive or null.")
    ...
    def select_action(self, predict_chunk: PredictChunk, observation: Any) -> Tensor:
        ...
        if not self._action_queue:
            ...
            self._action_queue.extend((index, action) for index, action in enumerate(action_chunk[0, :action_steps]))
...
class StrideActionDispatcher(ActionDispatcher):
    ...
    def __init__(self, dataset_hz: int, target_hz: int, queue_steps: int | None) -> None:
        if dataset_hz <= 0 or target_hz <= 0:
            raise ValueError("stride_action dataset_hz and target_hz must be positive.")
        if dataset_hz % target_hz != 0:
            raise ValueError("stride_action requires dataset_hz to be divisible by target_hz.")
        if queue_steps is not None and queue_steps <= 0:
            raise ValueError("stride_action.queue_steps must be positive or null.")
    ...
    def select_action(self, predict_chunk: PredictChunk, observation: Any) -> Tensor:
        ...
        if not self._action_queue:
            ...
            self._action_queue.extend((index, action_chunk[0, index]) for index in selected_indices)
            performed_inference = True
...
class TemporalEnsemblingDispatcher(ActionDispatcher):
    ...
    def select_action(self, predict_chunk: PredictChunk, observation: Any) -> Tensor:
        ...
        if self._ensembled_actions is None:
            ...
            self._ensembled_actions = torch.cat((self._ensembled_actions, action_chunk[:, -1:]), dim=1)
